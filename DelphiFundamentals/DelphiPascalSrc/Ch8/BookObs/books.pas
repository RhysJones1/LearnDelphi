...
{ also show extra data for HorrorBookObs and RefBookObs  }
var
   i :integer;
   s : BookString;
begin
{  ListBox1.Clear; }
  ...
  begin
         { find the TObject's class (using 'is' operator).
           if it is HorrorBookOb then display its gorescore.
           if it is RefBookOb then display its reftype }
      if TObject(obList.Items[i]) is HorrorBookOb then
         s := HorrorBookOb(obList.Items[i]).Describe
      else if TObject(obList.Items[i]) is RefBookOb then
         s := RefBookOb(obList.Items[i]).Describe
      else s:= BookOb(obList.Items[i]).Describe;
...
begin
  inherited Create;
  Form1.ListBox1.Items.Add( 'CONSTRUCTOR: BookOb.Create' );    // just for info
  name := aName;
  author := anAuthor;
end;
...
begin
  ...
  { If any fields of this class were objects, they would need to be
  created here:
  e.g. MyOb := MyObClass.Create; }
end;
...
begin
  ...
  { If any objects had been created they should be freed here:
  e.g. MyOb.Free;
  }
  name := '';
  author := '';
