...
type
  TfrmLoopingCharacters = class(TForm)
    ...
    procedure btnShowUpperAndLowerClick(Sender: TObject);
...
procedure TfrmLoopingCharacters.btnShowUpperAndLowerClick(Sender: TObject);
var
   chrCharacter : Char;
   blnReverse : Boolean;
begin
   blnReverse := chkReverse.Checked;
   lstUpperAndLower.Clear;

   if blnReverse = False then
   begin
      for chrCharacter := 'A' to 'z' do
      begin
        lstUpperAndLower.Items.Add(chrCharacter);
      end;
   end
   else
   begin
      for chrCharacter := 'z' downto 'A' do
      begin
        lstUpperAndLower.Items.Add(chrCharacter);
      end;
   end;

   pnlUpperAndLowerCount.Caption := IntToStr(lstUpperAndLower.Items.Count);
